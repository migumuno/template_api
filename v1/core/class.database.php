<?php
	...
	class db{
		...
		public function prebind($array){
			//comprobación de array para que no pete
			if(is_array($array)){
		...
		/**
		 * prepara los parámetros que vamos a usar en la query
		 * @param string $param es el valor de marcador de posición que vamos a utilizar en nuestra query, ejemplo :nombre.
		 * @param string $value es el valor que le vamos a dar al marcador de posicion de la query, ej Javier.
		 * @param string $type es el tipo de parámetro, example string.
		 */
		...
		/**
		 * Empieza una transacción a la que podemos volver atrás si algo falla
		 */
		...
		/**
		 * Vuelve atrás por si algo falla
		 */
		...
		/**
		 * Realiza una inserción o un update de manera automática
		 * en la tabla deseada en función de si se envÌa la pk o no
		 * @param array $datos 
		 * @param string $tabla con la que se interactúa
		...
		/**
		 * Te da la informacióón de una tabla.
		 * Uso exclusivo de la funcióón stor de mysql
		...
		/**
		 * stor asociado a las bases de datos mysql, mira descripcion de la función stor para saber quÈ hace y los parámetros
		 */
		private function mysql_stor($datos, $tabla, $pks=null){
		    ...
		    //puede que la tabla tenga más de una pk, si me lo pasan por parámetro hago caso a lo que me dicen
		    if(is_array($pks)){
		        //recorro las pks que me envían
		        foreach($pks as $pk){
		            ...
		            foreach($datos as $key=>$valor){
		            	//si me envian la pk
		                if($pk == $key){
		                	//si está vacío y no tenemos una de las pks rellenas
		            ...
		            //si me envían una pk puedo al menos hacer un update, si no
		            if($fill_pk == false){
		                $get->insert= true;
		            }
		        }
		    ...
		    //en función de si me viene o no es un insert o un update
	        if($get->insert){
		...
		/**
		 * stor asociado a las bases de datos mssql, mira descripcion de la función stor para saber qué hace y los parámetros
		 */
